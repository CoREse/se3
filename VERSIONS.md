...
- Support pytest-xdist verbose output in the TEST step parser so per-test results, SKIPPED detection, new-vs-regression classification and the critical-test guard stay accurate under parallel runs instead of silently falling back to summary counts
- Report a missing pytest-xdist as an actionable install error (both `python -m pytest` pre-flight and bare `pytest` argument-error detection) so it no longer enters the fix loop as an ordinary test failure
- Restrict the parallel switch to the primary test command only — commands in `test.phases` run exactly as written, and non-pytest commands are left untouched
- Document the new field in `tianluo.example.yaml` and the English and Chinese configuration references, including the `--dist loadgroup` side effect and the phases exemption
- Add a `can-be-coarse-but-never-wrong` accuracy discipline section to both discovery prompt templates, requiring every factual claim about the codebase to be verified against source in the current session and preferring stable references over line numbers
- Add a parallel-safety convention to the default charter conventions generated by `luo init` and `luo migrate`: generated tests should not depend on execution order, share mutable global state, or reuse fixed temporary paths
- Enable `test.parallel: auto` for this repository and pin the worktree test family to a single `xdist_group` serial group, with the marker registered in `pyproject.toml` so serial environments emit no unknown-marker warnings
## 12.6.0 - 2026-08-17
