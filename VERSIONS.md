...
**3.17.1** — fix: Fix JSON parsing bug in EXTRACT and TWO_PHASE modes. Return plain JSON string instead of stream-json wrapper. Remove wall time timeout from all LLM calls, use only inactivity timeout (30 minutes).

## Version History

| Version | Date | Changes |
|---------|------|---------|
| 3.17.1 | 2026-03-03 | fix: Fix JSON parsing bug in EXTRACT and TWO_PHASE modes. Return plain JSON string instead of stream-json wrapper. Remove wall time timeout from all LLM calls, use only inactivity timeout (30 minutes). |
