...
class TestMcpServer:
    """Test MCP collab server tool handlers."""
    ...
    def setup_method(self):
        ...
        # Set env vars
        os.environ["SE3_PROJECT_ROOT"] = self.tmpdir
        os.environ["SE3_TASK_ID"] = "task-test"
        os.environ["SE3_AGENT_ROLE"] = "worker"

        # Import the server module
        server_path = Path(__file__).parent.parent / "scripts" / "mcp-collab"
        ...
        # Update module-level paths
        self.server.PROJECT_ROOT = self.tmpdir
        self.server.COLLAB_DIR = Path(self.tmpdir) / ".collab"
        self.server.HUMAN_CALLS_DIR = Path(self.tmpdir) / "human-calls"
        self.server.ensure_dirs()

    def teardown_method(self):
        os.chdir(self.old_cwd)
        for key in ("SE3_PROJECT_ROOT", "SE3_TASK_ID", "SE3_AGENT_ROLE"):
            os.environ.pop(key, None)

    def test_notify_human(self):
        """notify_human should write event file and return immediately."""
        result = self.server.handle_tool_call("notify_human", {
        ...
        # Check event file was written
        events_dir = Path(self.tmpdir) / ".collab" / "events"
        event_files = list(events_dir.glob("*-notify-*.json"))
        assert len(event_files) == 1

        event = json.loads(event_files[0].read_text())
        assert event["message"] == "Starting implementation"
        assert event["level"] == "info"

    def test_report_progress(self):
        """report_progress should update task health info."""
        # Create a task file first
        tasks_dir = Path(self.tmpdir) / ".collab" / "tasks"
        tasks_dir.mkdir(parents=True, exist_ok=True)
        task = {"id": "task-test", "health": {}}
        (tasks_dir / "task-test.json").write_text(json.dumps(task))

        result = self.server.handle_tool_call("report_progress", {
            "task_id": "task-test",
        ...
        updated = json.loads((tasks_dir / "task-test.json").read_text())
        assert updated["health"]["last_message"] == "Halfway done"
        assert updated["health"]["percent"] == 50
        assert "last_activity" in updated["health"]

    def test_ask_human_writes_file(self):
        """ask_human should write a human-call file."""
        # We test only the file writing, not the blocking wait
        filepath = self.server.write_human_call(
        ...
        assert "What auth method should we use?" in content
        assert "high" in content  # urgent
        assert "OAuth2" in content
        assert "JWT" in content

    def test_unknown_tool(self):
        """Unknown tool should return error."""
        result = self.server.handle_tool_call("nonexistent_tool", {})
        assert result.get("isError") is True

    def test_tools_list(self):
        """Should have exactly 3 tools defined."""
        assert len(self.server.TOOLS) == 3
        tool_names = {t["name"] for t in self.server.TOOLS}
        assert tool_names == {"ask_human", "notify_human", "report_progress"}
