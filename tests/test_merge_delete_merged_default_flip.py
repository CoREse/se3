...
* archive its bound worktree under
  ``<project_root>/.se3/archive/<slug>-<ts>/`` before the destructive
  removal.
...
def test_run_merge_default_deletes_branch_and_archives_worktree(
    ...
    is archived to ``.se3/archive/`` before deletion.
    """
    ...
    # Archive should exist under .se3/archive/feature-<ts>/.
    archives = _archive_dirs(tmp_path)
    assert archives, "expected an archive directory under .se3/archive/"
    archived_for_feature = [d for d in archives if d.name.startswith("feature-")]
    assert archived_for_feature, (
        f"no feature-* archive found under .se3/archive/; saw: {archives}"
    )
