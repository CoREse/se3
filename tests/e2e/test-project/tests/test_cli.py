...
class TestSearchCommand:
    ...
    def test_search_no_tasks(self):
        """Test searching when no tasks exist."""
        result = self.runner.invoke(cli, ["search", "test"])
        assert result.exit_code == 0
        assert "No tasks found matching 'test'" in result.output
    
    def test_search_no_matches(self):
        """Test searching with no matching tasks."""
        self.runner.invoke(cli, ["add", "Buy milk"])
        self.runner.invoke(cli, ["add", "Walk dog"])
    ...
    def test_search_case_insensitive(self):
        ...
        result = self.runner.invoke(cli, ["search", "milk"])
        assert result.exit_code == 0
    ...
    def test_search_multiple_matches(self):
        """Test search returns multiple matching tasks."""
        self.runner.invoke(cli, ["add", "Buy milk"])
        self.runner.invoke(cli, ["add", "Buy eggs"])
        self.runner.invoke(cli, ["add", "Walk dog"])
        
        result = self.runner.invoke(cli, ["search", "Buy"])
        assert result.exit_code == 0
        assert "Buy milk" in result.output
        assert "Buy eggs" in result.output
        assert "Walk dog" not in result.output
        assert "Found 2 task(s)" in result.output
    
    def test_search_partial_match(self):
        """Test search matches partial strings."""
        self.runner.invoke(cli, ["add", "Buy organic milk from store"])
        
        result = self.runner.invoke(cli, ["search", "milk"])
        assert result.exit_code == 0
        assert "Buy organic milk from store" in result.output

    def test_search_found(self):
        """Test normal search returns matching tasks."""
        # Create tasks with known titles
        self.runner.invoke(cli, ["add", "Buy milk"])
        self.runner.invoke(cli, ["add", "Buy eggs"])
        self.runner.invoke(cli, ["add", "Walk dog"])

        # Search for keyword that matches some tasks
        result = self.runner.invoke(cli, ["search", "Buy"])

        # Assert matching tasks appear in output
        assert result.exit_code == 0
        assert "Buy milk" in result.output
        assert "Buy eggs" in result.output
        assert "Found 2 task(s)" in result.output
    ...
    def test_search_not_found(self):
        """Test appropriate message when no tasks match."""
        # Create some tasks
        self.runner.invoke(cli, ["add", "Buy milk"])
        self.runner.invoke(cli, ["add", "Walk dog"])

        # Search for keyword that matches no tasks
        result = self.runner.invoke(cli, ["search", "nonexistent"])

        # Assert friendly 'no results' message is displayed
        assert result.exit_code == 0
        assert "No tasks found matching 'nonexistent'" in result.output
    ...
    def test_search_case_insensitive(self):
        ...
        # Assert match is found despite case difference
        assert result.exit_code == 0
        assert "buy milk" in result.output
        assert "Found 1 task(s)" in result.output
