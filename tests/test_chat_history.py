...
class TestRenderSessionDetailed:
    ...
    def test_prompt_segmentation_in_panel(self):
        """Prompt section should contain segment titles."""
        from rich.console import Console
        from io import StringIO
        session = self._make_session()
        renderables = render_session_detailed(session, verbose=False)
        buf = StringIO()
        c = Console(file=buf, force_terminal=False, width=200)
        # Prompt section spans the first 4 renderables (heading + blank + body + blank)
    ...
    def test_markdown_heading_attempt_label(self):
        ...
        messages = [
            ChatMessage(role="user", content="Q1", raw_json=[],
                        timestamp="2026-01-01T12:00:00",
                        step_type="analyze", attempt=0),
            ChatMessage(role="assistant", content="First.",
                        ...
                        timestamp="2026-01-01T12:00:05",
                        step_type="analyze", attempt=0),
            ChatMessage(role="user", content="Q2", raw_json=[],
                        timestamp="2026-01-01T12:01:00",
                        step_type="analyze", attempt=1),
            ChatMessage(role="assistant", content="Second.",
                        raw_json=[ndjson_dict2],
