"""Existence tests for the `luo code-index search` prompt-guidance rollout.

Issue #262 Part 2: every LLM-facing place that teaches the code-index commands
must also advertise `luo code-index search <pattern>` as the grep-replacement
for locating code-index items (a raw grep of `tianluo/code-index.md` loses a
...
SEARCH_CMD = "luo code-index search"
...
GREP_MARKERS = ("grep", "tianluo/code-index.md")
...
def test_code_index_injection_header_advertises_search(tmp_path: Path) -> None:
    ...
    # No tianluo/code-index.md under tmp_path -> the "not built" branch, whose
