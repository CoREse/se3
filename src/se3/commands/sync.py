...
def process_call_response(
    ...
    """Process a sync respond-channel response file.

    The single-directional sync flow emits three call-file types, all confirmed
    through this respond channel: ``sync_high_impact_deletion`` (whole-Requirement
    removal), ``sync_base_migration`` (relocating module-level Requirements out of
    ``base``), and ``sync_spec_split`` (splitting a multi-topic spec into a
    parallel spec). Legacy ``sync_pending_decisions`` / ``sync_conflicts`` formats
    are no longer supported — those produce a clear error pointing the user at the
    new workflow.
    """
    ...
    extra = ""
    if result.get("specs_created"):
        extra = f"\nNew specs created: {', '.join(result['specs_created'])}"
    if result.get("migrated"):
        moved = ", ".join(
            f"{m.get('requirement_name')} → {m.get('target_spec')}"
