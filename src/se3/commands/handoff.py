...
"""
        ...
        if not has_changes:
            ...
            # Commit progress.md changes if any
            progress_file = root / "progress.md"
            result = run_command(["git", "diff", "--quiet", str(progress_file)], cwd=root)
            if result.returncode != 0:  # progress.md has changes
                typer.echo("  Committing progress.md...")
                run_command(["git", "add", str(progress_file)], cwd=root)
                run_command(
                    ["git", "commit", "-m", "chore(progress): update session history\n\nAuto-generated by se3 handoff."],
                    cwd=root
                )
                typer.echo("  progress.md committed.")
