...
class SpecWriteProtectionConfig:
    ...
    Fields:
        hook_enabled: Install the PreToolUse spec-write hook (the primary,
            real-time block) for non-exempt steps. Default ``True``.
        diff_fallback_enabled: Run the post-step spec-diff fallback (the backstop
            that catches Bash-redirect writes) for non-exempt steps.
            Default ``True``.
    """

    hook_enabled: bool = DEFAULT_SPEC_WRITE_HOOK_ENABLED
    diff_fallback_enabled: bool = DEFAULT_SPEC_WRITE_DIFF_FALLBACK_ENABLED

    @staticmethod
    def _coerce_bool(data: dict, key: str, default: bool) -> bool:
        """Return ``data[key]`` as a bool, raising on a non-boolean value."""
        if key not in data:
            return default
        raw = data[key]
        if not isinstance(raw, bool):
            raise ConfigError(
                f"spec_write_protection.{key} must be a boolean (true/false), "
                f"got {type(raw).__name__} ({raw!r})"
            )
        return raw

    @classmethod
    def from_dict(cls, data: dict) -> "SpecWriteProtectionConfig":
        """Create from the ``spec_write_protection`` YAML section."""
        if not data or not isinstance(data, dict):
            return cls()
        return cls(
            hook_enabled=cls._coerce_bool(
                data, "hook_enabled", DEFAULT_SPEC_WRITE_HOOK_ENABLED
            ),
            diff_fallback_enabled=cls._coerce_bool(
                data, "diff_fallback_enabled", DEFAULT_SPEC_WRITE_DIFF_FALLBACK_ENABLED
            ),
        )

    @classmethod
    def load(cls, project_root: Optional[Path] = None) -> "SpecWriteProtectionConfig":
        """Load from the active project YAML, falling back to defaults."""
        if project_root is None:
            return cls()
        data, _src = load_project_yaml(project_root)
        if not data:
            return cls()
        section = data.get("spec_write_protection")
        if section is None:
            return cls()
        if not isinstance(section, dict):
            raise ConfigError(
                "spec_write_protection: expected a mapping, got "
                f"{type(section).__name__}"
            )
        return cls.from_dict(section)


def load_spec_write_protection_config(
    project_root: Optional[Path] = None,
) -> SpecWriteProtectionConfig:
    """Load spec-write-protection configuration.

    Args:
        project_root: Project root directory. ``None`` returns the built-in
            default (both layers enabled).

    Raises:
        ConfigError: If ``spec_write_protection`` is set but a value is not a
            boolean (or the section is not a mapping).
    """
    return SpecWriteProtectionConfig.load(project_root)
