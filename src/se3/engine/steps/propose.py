...
def propose_handler(step: Step, flow: FlowInstance) -> StepStatus:
    ...
    try:
        ...
        # Parse JSON response
        proposal = _parse_propose_response(response)
...
def _parse_propose_response(response: str) -> dict[str, Any] | None:
    ...
    try:
        ...
        if "summary" not in result:
            logger.warning("Missing 'summary' in proposal response")
            return None
