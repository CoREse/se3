...
def update_spec_handler(step: Step, flow: FlowInstance) -> StepStatus:
    ...
    try:
        ...
        # Parse JSON response
        update_plan = _parse_update_response(response)
...
def _parse_update_response(response: str) -> dict[str, Any] | None:
    """Parse the LLM response into structured update plan."""
    try:
        ...
        if "specs_to_update" not in result:
            logger.warning("Missing 'specs_to_update' in update_spec response")
            return None
