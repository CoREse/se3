...
# K5/K6: special repository state errors ------------------------------------
...
class MergeOrchestrator:
    ...
    def __init__(
        ...
        # cache and retry budget across all phases of one merge.
        ...
        # K2: per-LLM-call jsonl trace for observability.
    ...
    def _log(
        ...
        K3: ``level`` lets error paths emit at ERROR/WARNING while the
        internal transcript buffer records every line regardless of level.
        """
        ...
        if effective_pre_merge_version and branch_bumps:
            ...
            try:
                ...
                if agg.success:
                    ...
                    self._log(f"Version aggregation failed: {agg.error}", level=logging.ERROR)
    ...
    def _infer_bump_for_branch(
        ...
        try:
            ...
            self._log(f"Failed to infer bump for '{branch}': {exc}", level=logging.ERROR)
            return None
    ...
    def execute(self, branches: list[str]) -> MergeReport:
        ...
        try:
            ...
            self._log(f"Merge lock contention: {exc}", level=logging.ERROR)
            self._write_log()
    ...
    def _execute_locked(self, branches: list[str]) -> MergeReport:
        ...
        # K5/K6: fail-fast on unsupported repo states
        try:
            _check_repo_state(self.project_root)
        except (EmptyRepoError, DetachedHeadError, ShallowRepoError) as exc:
            self._log(str(exc), level=logging.ERROR)
            report.set_failure_reason(FailureReason.MERGE_FAILED)
        ...
        for idx, branch in enumerate(branches):
            ...
            if result == "merged" or result == "already_merged":
                ...
                self._log(f"Branch '{branch}' has conflicts — aborting", level=logging.ERROR)
                if report.merged_branches:
                    self._log(
                ...
                self._log(f"Branch '{branch}' merge failed (non-conflict) — aborting", level=logging.ERROR)
                if report.merged_branches:
                    self._log(
                        f"Version not bumped despite {len(report.merged_branches)} "
                        f"successful merge(s) — re-run after resolving"
        ...
        if self.delete_merged and report.success:
            ...
            try:
                ...
                self._log(f"Cleanup failed: {exc}", level=logging.ERROR)
    ...
    def _merge_single_branch(self, branch: str, report: MergeReport) -> str:
        ...
        if result.returncode == 0:
            ...
            try:
                ...
                self._log(f"Rollback failed after guardrail violation: {exc}", level=logging.ERROR)
                report.rollback_failed = True
    ...
    def _handle_conflict(
        ...
        try:
            ...
            self._log(f"Failed to build conflict context: {exc}", level=logging.ERROR)
            abort_ok = self._abort_merge()
            if self.strategy == MergeStrategy.FAST:
                if not abort_ok:
                    report.failure_reason = "merge_abort_failed"
        ...
        try:
            ...
            self._log(f"LLM resolution failed: {exc}", level=logging.ERROR)
            if self.strategy == MergeStrategy.FAST:
                if not self._abort_merge():
                    report.failure_reason = "merge_abort_failed"
                else:
    ...
    def _apply_resolution(
        ...
        if add_failures:
            ...
            self._log("Aborting merge due to validation failures", level=logging.ERROR)
            abort_ok = self._abort_merge()
        ...
        try:
            ...
            if add_failures:
                self._log("Aborting merge due to write/stage failures", level=logging.ERROR)
                abort_ok = self._abort_merge()
                if not abort_ok:
                    report.failure_reason = "merge_abort_failed"
                    if self.strategy == MergeStrategy.FAST:
        ...
        if commit_result.returncode != 0:
            self._log(f"Merge commit failed: {commit_result.stderr.strip()}", level=logging.ERROR)
            abort_ok = self._abort_merge()
            if not abort_ok:
                report.failure_reason = "merge_abort_failed"
                if self.strategy == MergeStrategy.FAST:
    ...
    def _run_guardrails(
        ...
        if not pre_sha or not post_sha:
            ...
            try:
                ...
                self._log(f"Failed to write guardrail human call file: {exc}", level=logging.ERROR)
                if pre_sha:
                    call_err_msg = (
        ...
        try:
            ...
            self._log(f"Guardrails check failed for '{branch}': {exc}", level=logging.ERROR)
            if strategy == MergeStrategy.FAST:
            ...
            try:
                ...
                self._log(f"Failed to write guardrail human call file: {write_exc}", level=logging.ERROR)
                raise GuardrailCallFileError(
    ...
    def _abort_merge(self) -> bool:
        ...
        try:
            ...
            self._log(f"git merge --abort timed out: {exc}", level=logging.ERROR)
            return False
        if abort_result.returncode == 0:
            self._log("git merge --abort succeeded")
            return True
        else:
            self._log(f"git merge --abort failed: {abort_result.stderr.strip()}", level=logging.ERROR)
            return False
    ...
    def _rollback_to(self, sha: str) -> None:
        ...
        if reset_result.returncode == 0:
            ...
            self._log(f"git reset --hard failed: {error_msg}", level=logging.ERROR)
            raise RuntimeError(
