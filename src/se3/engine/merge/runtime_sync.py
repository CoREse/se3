...
def _atomic_write_bytes(dest_path: Path, content: bytes) -> None:
    ...
    Eliminates the practical symlink-takeover risk path described in
    E1/E5; a tiny TOCTOU window between the lstat and the rename
    remains, but during that window the worst case is that rename
    replaces the just-planted symlink (rename does not follow it), so
    the write target is bounded to *dest_path* itself either way.
    Mirroring this lstat guard at the parent-directory level would
    require fd-based traversal (``openat`` per component); given the
    destination lives under user-controlled ``project_root/se3/`` the
    practical risk is low and the closed parent-symlink gap is
    acknowledged here for future hardening.
    ...
    try:
        ...
        # the tiny window between our initial lstat and the rename, the
        ...
        # nanosecond-scale TOCTOU window between this lstat and the
        # rename, acknowledged in the docstring; a fully robust fix
        # requires fd-based renameat with directory file descriptors.
        try:
        ...
        # os.rename does not follow symlinks at either side — even if a
        # symlink were planted between the recheck and this call, rename
        # atomically replaces the path entry rather than following it.
