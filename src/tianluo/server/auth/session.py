...
- **High entropy** — a session id is a 256-bit random token
  (:func:`tianluo.server.crypto.generate_token`); guessing one is infeasible, so a
...
- **Constant-time validation** — lookup matches the presented id's hash against
  the stored hash via :func:`tianluo.server.crypto.const_eq`, avoiding a timing
...
class SessionAuthProvider(AuthProvider):
    ...
    session (the default :class:`~tianluo.server.auth.local.LocalAuthProvider`, and
