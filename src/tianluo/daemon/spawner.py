...
def _resolve_se3_command() -> List[str]:
    """Return the argv prefix that invokes the ``tianluo`` CLI.
    ...
    up a different environment's console script via ``PATH``:

    1. ``Path(sys.executable).parent / <script>`` for ``luo`` / ``tianluo`` /
       the transitional ``se3`` alias (plus ``.exe`` variants on Windows).
       ...
       ``luo run`` / ``luo init`` on behalf of a remote caller.
    2. ``[sys.executable, '-m', 'tianluo']``. When no console script is
       present next to the interpreter (broken install, renamed script),
       ``-m tianluo`` still loads the package via the same interpreter and
       therefore the same site-packages — bypassing ``PATH`` entirely.
    3. ``shutil.which()`` over the same script names as a last-resort
       fallback for exotic packagings where neither of the above works.
    """
    script_names = ["luo", "tianluo", "se3"]
    if sys.executable:
        bin_dir = Path(sys.executable).parent
        candidates = [bin_dir / name for name in script_names]
        if os.name == "nt":
            candidates.extend(bin_dir / f"{name}.exe" for name in script_names)
        for candidate in candidates:
            if candidate.is_file():
                return [str(candidate)]
        return [sys.executable, "-m", "tianluo"]
...
def _plan_mode_args(decomposition: str, granularity: str) -> List[str]:
    """Return the plan-mode CLI options for a spawn, omitting the unset ones.

    An absent option is not the same as a default one: leaving it off the argv
    is what lets the project's own ``workflow.plan_decomposition`` /
    ``plan_granularity`` decide, whereas passing a guessed value would overrule
    the project with something the operator never chose.
    """
    args: List[str] = []
    if decomposition:
        args += ["--plan-decomposition", str(decomposition)]
    if granularity:
        args += ["--plan-granularity", str(granularity)]
    return args
...
class DaemonSpawner:
    ...
    def __init__(
        ...
        *,
        ...
        Args:
            supervisor: Optional :class:`~tianluo.daemon.supervisor.DaemonSupervisor`.
    ...
    def spawn(
        ...
        *,
        ...
        When *plan_decomposition* / *plan_granularity* are non-empty, the
        matching ``--plan-decomposition <value>`` / ``--plan-granularity
        <value>`` options are appended (fresh and from-issue paths alike) so
        the operator's explicit web choice reaches the CLI. Empty means "no
        option on the argv" — the CLI then resolves the project configuration /
        default, never a value the server guessed.
        ...
        **not** placed on the argv — the CLI's ``--from-issue`` path takes the
        ...
        if from_issue_id:
            ...
            args += _plan_mode_args(plan_decomposition, plan_granularity)
            if extra_args:
