...
:class:`DaemonAggregator` polls the structured artifacts that ``luo run``
leaves on disk — ``tianluo/state/engine.json``, ``tianluo/state/summary-*.json``,
``tianluo/calls/``, ``tianluo/logs/``, ``tianluo/issues/`` — and folds them into a single
...
files those flows write. This keeps it decoupled from the ``luo run`` process
...
# whole ``tianluo/history`` tree (reading every ``_meta.json``) and is far too
...
class PendingCall:
    ...
    Mirrors a file under a project's ``tianluo/calls/`` directory — the unified
...
class FlowSnapshot:
    ...
    # ``tianluo/state/resumable/<flow_id>.json`` snapshot that has been superseded by
...
class DaemonAggregator:
        ...
        *,
        ...
        Args:
            ...
            live_roots_provider: Optional zero-arg callable returning the set of
                project roots that currently have a *live* ``luo run`` process,
        ...
        # (no active flow last fast tick) and its ``tianluo/state/.dirty`` sentinel
        ...
        # under ``tianluo/calls/``. ``_last_calls_signature`` holds the last emitted
    ...
    def add_project_root(self, path: object) -> None:
        ...
        when no ``luo run`` process is currently live. Persistence is
        ...
        ``<main>/tianluo/worktrees/<name>`` isolation sandbox is folded back to its
        ...
        # full ``tianluo/history`` walk every tick (the exact high-frequency disk
    ...
    def get_snapshot(self) -> MachineStatus:
        ...
        # daemon restart with no live ``luo run`` process, a FAILED/PAUSED
        ...
        # ``luo run --worktree`` subdirectory so an isolation run gets a live
        ...
        for root_str in observable_roots:
            ...
            # issue YAML files under tianluo/issues/; skipping collection
        ...
        # ``tianluo/state/resumable/<flow_id>.json`` (see PersistenceManager); a
        # later ``luo run`` overwrites the single-slot engine.json but leaves
    ...
    def all_project_roots(self) -> List[str]:
        ...
        * the disk-history roots discovered by
        ...
        full ``tianluo/history`` walk on every status tick is what previously stalled
    ...
    def all_observable_roots(self) -> List[str]:
        ...
        ``luo run --worktree`` flow is observable in the WebUI — flow card,
    ...
    def _active_worktree_run_roots(self) -> List[str]:
        """Discover ``luo run --worktree`` isolation subdirectories to observe.

        A ``--worktree`` run executes its entire flow body inside
        ``<main_repo>/tianluo/worktrees/<name>/``, persisting its own
        ``tianluo/state/engine.json`` and ``tianluo/history/<flow_id>/`` there — never
        ...
        This scans every tracked main root's ``tianluo/worktrees/*/`` for a child
        ...
        ``tianluo/worktrees/`` parent directory but never write a top-level
        ...
        seen: Set[str] = set()
        for base in bases:
            worktrees_dir = runtime_dir(Path(base)) / "worktrees"
            try:
            ...
            for entry in entries:
                ...
                data = read_engine_header(
                    runtime_dir(entry) / "state" / "engine.json", active=True
                )
    ...
    def _historical_roots(self, base: Set[str]) -> List[str]:
        ...
        full ``tianluo/history`` walk off the per-tick hot path.
        """
    ...
    def pending_calls_signature(self) -> Dict[str, Any]:
        """Return a cheap stat-based fingerprint of every ``tianluo/calls/`` file.

        For each registered project root, the fingerprint lists every regular
        file under ``tianluo/calls/`` as ``(name, mtime, size)``. Listing the
        ...
        # clarification) under ``<worktree>/tianluo/calls/``, so without the worktree
    ...
    def _snapshot_for_root(
        ...
        *live_roots* (when supplied) is the normalized set of roots with a live
        ``luo run`` process; it gates the ``resumable`` flag of a ``RUNNING``
        ...
        log_count = _count_dir(runtime_dir(root) / "logs")
        issue_count = _count_issues(runtime_dir(root) / "issues")
    ...
    def _enumerate_resumable_snapshots(
        ...
        Enumerates ``<root>/tianluo/state/resumable/*.json`` — the per-flow
        ...
        """
        resumable_dir = runtime_dir(root) / "state" / "resumable"
    ...
    def _snapshot_from_resumable(
        ...
        ``tianluo/calls`` & ``tianluo/issues``, not to this superseded snapshot.
        ...
        ``RUNNING`` snapshot whose root still has a live ``luo run`` process is
    ...
    def _enumerate_calls(self, root: Path) -> List[PendingCall]:
        """List genuinely pending human-call files under ``tianluo/calls/``.

        An answered call's ``.json`` request file and its sibling
        ``.response`` / ``.response.json`` answer file both linger in the
        directory indefinitely (``luo history`` and friends rely on them).
    ...
    def _filter_calls_for_flow(
        ...
        ``sync_conflicts_*.json``, …) that linger in ``tianluo/calls/`` and must
    ...
    def _filter_stale_calls(
        ...
        ``luo run`` loop consumes the terminal answer directly and advances. So
    ...
    def _parse_call_file(entry: Path, root: Path) -> PendingCall:
        """Build a :class:`PendingCall` from one ``tianluo/calls/`` file.
    ...
    def _collect_issues(self, root: Path) -> List[IssueSnapshot]:
        """Read issue YAML files from ``tianluo/issues/`` and return snapshots.
        ...
        An ``luo run --worktree`` isolation directory clones the main project's
        ``tianluo/issues/`` into ``<main>/tianluo/worktrees/<name>/tianluo/issues/``.
        ...
        cache_key = str(root)
        issues_dir = runtime_dir(root) / "issues"
...
def _normalize_root(path: object) -> str:
    ...
    Folds a worktree-copy sandbox (``<main>/tianluo/worktrees/<name>``) back to its
...
def _resumable_with_live_gate(
    ...
    ``luo run`` process for that root — the flow is reported NOT resumable, so
...
def _count_issues(issues_dir: Path) -> int:
    """Count open issue records under ``tianluo/issues/`` (``open/`` subtree)."""
    if not issues_dir.is_dir():
        return 0
