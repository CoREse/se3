...
# All console-script names the CLI has shipped under (``luo``/``tianluo``
# since the rename, ``se3`` as the transitional alias) and the module names
# accepted by ``python -m``. A supervisor must recognise processes launched
# through any of them — old and new installs coexist during the 12.x window.
_CLI_PROGRAM_NAMES = frozenset({"luo", "tianluo", "se3"})
_CLI_MODULE_NAMES = frozenset({"tianluo", "se3"})


def _cmdline_is_se3_run(cmdline: List[str]) -> bool:
    """Structurally decide whether *cmdline* is a real ``tianluo run`` invocation.
    ...
    *interpreter* option, so it precedes any program/module token, and
    ...
    for i in (0, 1):
        if (
            i + 1 < len(cmdline)
            and os.path.basename(cmdline[i]) in _CLI_PROGRAM_NAMES
            and cmdline[i + 1] == "run"
        ):
            return True
    ...
    if "-m" in cmdline:
        ...
        if (
            ...
            and cmdline[idx + 1] in _CLI_MODULE_NAMES
