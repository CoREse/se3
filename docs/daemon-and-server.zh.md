...
- **向运行中的流程插话。** **Interject** 按钮把一条指令推给正在调用中的流程。
  这不再是把一段文字追加到下一个 step:运行进程会优雅地停下进行中的 agent 调用,
  并开启一场*插话对话* —— 在干活的那个 agent 自己的 provider 会话里、以只读方式
  与它对话。对话的每一轮都以一条 `dialog` 待处理调用浮现,携带到目前为止的
  transcript;你用自由文本回复即可继续对话,一旦 agent 给出决定(`continue` /
  `restart` / `exit`,可带一条一次性指令、一份完整的新任务描述、回退目标以及
  `keep`/`reset` 的工作区处理),面板会把每个字段渲染成可编辑控件,外加一个
  **执行该决定** 按钮。在你点下之前什么都不会执行。由于这些字段可编辑,把提案改成
  `restart` + `reset` 时会先取一份预览,列出该 reset 将丢弃的工作区改动与本 flow
  的提交;只有该预览显示出来之后,才能真正执行。**执行该决定** 按钮始终发送你看得见
  的那些字段,因此即便终端侧此后改过这一轮,执行的仍是你批准的内容;而*不带字段*的
  确认(非 WebUI 客户端可能发送的裸 `confirm` 选项)则被绑定到它所回应的那一轮:
  该轮次此后被改动过时,这次确认会被拒绝,并重新发布当前轮次以请求重新确认。
  重发同一条不带字段的确认并不会因此生效 —— 请求超时后盲目重试的客户端正是如此,
  它从未看见新一轮。这类客户端要确认,需在 `confirm` 之外回显该轮次的
  `decision_revision`(发布在调用 context 中),把答复绑定到它真正渲染过的那一轮。
  对已经*暂停*的流程(DISCOVERY
  提问、确认门、Retry/Skip/Abort 门)插话同样有效:daemon 会唤醒该暂停流程,使这条
  消息立刻被消费,而不必等某个无关的回复把它带醒。对暂停在 DISCOVERY 的流程插话
  则是刻意不同的:此时你本就在一个对话提示符前,所以这段文字会直接作为该 discovery
  的下一条回复投递。
