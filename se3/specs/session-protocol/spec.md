...
**Resumable Criterion (flow-semantic):** A flow SHALL be treated as resumable whenever it has **not** reached a normal `COMPLETED` and a valid intermediate state still exists for it, independent of any particular storage slot. This covers three classes: (a) a flow **paused** for a user interjection / awaiting input (`PAUSED`); (b) a flow that **exited mid-flow / was interrupted** (a non-normal termination — including a process killed while in the `discovery` or any other step — whose persisted status is typically `RUNNING`), qualifying as resumable **only when its `project_root` no longer holds a live `se3 run` process**; and (c) a flow **aborted on error** whose state is still recoverable (`FAILED`). Only a normally `COMPLETED` flow is the terminal non-resumable state. The judgement is by flow semantics, not by which on-disk artifact happens to hold the state — **with the one live-process caveat on class (b)**: a flow whose persisted status is `RUNNING` but whose process is **still alive** is genuinely executing, NOT recoverable-and-stalled, so it is excluded from the resumable set. Such a flow is rejected by `request_resume`'s live-process guard (it cannot be double-spawned), and the daemon/server therefore report its `resumable` flag as `False` so the web console shows **no** clickable Resume entry for it (see the `daemon` *Daemon Modules* live-process resumable gate). This caveat narrows class (b) **only** — it removes "the process is genuinely still alive" from the resumable set while leaving the interrupted/dead-process `RUNNING` flow resumable; `PAUSED` (class a, whose process is typically still alive waiting for input) and `FAILED` (class c) judgements are unchanged.
