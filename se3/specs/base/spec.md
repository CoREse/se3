...
- `merge/` — merge 相关支撑模块子包，供 `merge_cmd.py` 与 `engine/merge/` 使用：
  ...
  - `llm_trace.py` — `se3 merge` 期间每次 LLM 调用的 JSONL trace 日志，append-only；每条记录写入后 `flush`（即时经页缓存可见），但 `os.fsync` 改为批量 / 降频（默认每 N 条或 T 秒强制一次，并在 `stop()` / 文件轮转 / 进程退出时强制一次），以缓解多个并行 merge 组流式写 trace 叠加 daemon 每 tick stat 全部文件造成的脏页回写 IO 放大；输出至 `se3/logs/llm/merge_<timestamp>_<seq>.jsonl`
