...
# Loop mode (continuously execute tasks)
se3 run --loop
...
#### Scenario: Loop mode
- **WHEN** the user executes `se3 run --loop`
- **THEN** after completing one task, the flow engine continues to execute the next task

#### Scenario: Loop mode branch isolation
- **WHEN** the user executes `se3 run --loop` (without `--no-worktree`)
- **THEN** call `create_loop_branch(project_root, task_id=..., iteration=...)` to create a `loop/{task_id}-{iteration}` branch from the current HEAD (task_id is generated by slugifying the task description and truncating it to 30 characters; when the slugify result is an empty string, it falls back to `task`)
- **AND** create a git worktree at `se3/worktrees/{branch_safe_name}`
- **AND** all tasks execute within the worktree (file reads/writes and commits all happen inside the worktree)
- **AND** after the loop ends, prompt the user to choose: merge / later / discard
- **NOTE** Backward compatibility: `list_loop_branches()` matches both the legacy format `se3-loop/*` (marked as `[legacy]`) and the new format `loop/*`
...
- **WHEN** the user executes `se3 run --loop --no-worktree`
- **THEN** all tasks execute directly on the current branch (no branch isolation)

#### Scenario: Deferred Merge
- **WHEN** the user executes `se3 run --loop --merge loop/fix-auth-1` (or the legacy format `se3-loop/20260324-120000`)
- **THEN** display a diff summary and, after confirmation, merge the specified loop branch into the current branch
- **AND** if there are conflicts, handle them according to the `conflict_resolver.strategy` configuration (see se3-config spec)

#### Scenario: List loop branches
- **WHEN** the user executes `se3 run --list-loops`
- **THEN** display all unmerged loop branches and their commit counts
- **AND** if there are no loop branches, indicate that there are none

#### Scenario: Loop mode external wrapper architecture
- **WHEN** `se3 run --loop` executes loop iterations
- **THEN** the outer `LoopController` manages the branch/worktree lifecycle, task discovery, and iteration counting
- **AND** the inner `run_flow()` executes the standard 10-step flow, unaware of loop mode
- **AND** loop context is injected into LLM calls only through `set_extra_prompt(persistent=True)`
- **AND** the persistent prompt is retained across multiple LLM calls and cleaned up after the iteration ends

#### Scenario: Loop mode task re-selection avoidance
- **WHEN** a task fails during a loop iteration
- **THEN** that task is added to the `failed_tasks` set
- **AND** subsequent iterations automatically skip already-completed and already-failed tasks
...
| `is_loop_mode` | `bool` | `False` | True when the flow was started via `se3 run --loop`. Distinguishes loop-mode flows for resume routing, history filtering, and the loop-iteration prompt prefixing described in *Loop-Mode External Wrapper Architecture*. |
...
| `loop_original_branch` | `str \| None` | `None` | The branch that was checked out when `se3 run --loop` was invoked. Captured so the loop controller can offer `merge` / `later` / `discard` against the correct destination after the iteration finishes (see *Deferred Merge* scenario). |
...
Options:
  ...
  --merge BRANCH    Merge an existing loop branch (e.g. loop/fix-auth-1 or the legacy format se3-loop/20260324-120000)
```
